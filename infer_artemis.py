...
# from vision_process import process_vision_info  # 请确保这个模块存在
...
def get_chunk(lst, n, k):
    """Get the k-th chunk after splitting lst into n parts."""
...
def process_questions(questions, answers_file, num_chunks, chunk_idx):
    ...
    # 从question中删除"image"，”sent“以及"bbox"一样的项目
    existing_keys = {(q["image"], q["sent"], tuple(q["bbox"])) for q in existing_answers}
    new_questions = [q for q in questions if (q["image"], q["sent"], tuple(q["bbox"])) not in existing_keys]
...
def main(args):
    ...
    # Load and chunk questions (no shuffle)
    ...
    for mini_batch in tqdm(list(batch(questions, bs)), total=math.ceil(len(questions)/bs)):
        ...
        for line in mini_batch:
            query = build_prompt(line["sent"], "grounding")
